...
outfile = "/Users/liziling/Desktop/Duke/fintech 545/Assignment 2/testout6_2.csv"
...
#  r_t = log(P_t / P_{t-1}) =====
function log_returns(df_prices::DataFrame)
    n, m = size(df_prices)
    n < 2 && error("lack of sample")
    ret = DataFrame()
    for cname in names(df_prices)
        ...
        @inbounds for i in 2:n
            p0, p1 = col[i-1], col[i]
            r[i-1] = (ismissing(p0) || ismissing(p1)) ? missing : log(p1 / p0)
        end
        ret[!, cname] = r
    end
    return ret
